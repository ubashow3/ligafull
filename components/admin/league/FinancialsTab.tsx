...
import { League, Championship, ChampionshipFinancials } from '../../../types';
...
const FinancialsTab: React.FC<FinancialsTabProps> = ({ league, onSaveFinancials }) => {
  ...
  const selectedChampionship = useMemo(() => {
    return league.championships.find(c => c.id === selectedChampId);
  }, [selectedChampId, league.championships]);

  useEffect(() => {
    if (selectedChampionship) {
      setFormState(selectedChampionship.financials || emptyForm);
    } else {
      setFormState(emptyForm);
    }
  }, [selectedChampionship]);

  const isReadyForFinancials = selectedChampionship && selectedChampionship.clubs.length > 0 && selectedChampionship.matches.length > 0;
  ...
  const calculatedValues = useMemo(() => {
    ...
    const registrationFeePerClub = selectedChampionship.clubs.length > 0 ? totalCost / selectedChampionship.clubs.length : 0;
    
    return { totalCost, registrationFeePerClub };
  }, [isReadyForFinancials, formState, selectedChampionship]);
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    onSaveFinancials(selectedChampId, finalFinancials);
  ...
  return (
    <div className="animate-fade-in">
        ...
        <div className="bg-gray-800 p-4 sm:p-6 rounded-lg">
             <div className="mb-4">
                <label htmlFor="championship-select" className="block text-sm font-medium text-gray-300 mb-1">Selecione o Campeonato</label>
                <select
             ...
             {selectedChampId && !isReadyForFinancials && (
             ...
             {isReadyForFinancials && selectedChampionship && (
                    ...
                    <div className="border-t border-gray-700 pt-4">
                        ...
                        <div className="space-y-2 text-sm">
                            <div className="flex justify-between p-2 bg-gray-900/50 rounded">
                                ...
                                <span className="font-bold text-white">{selectedChampionship.matches.length}</span>
                            </div>
                             <div className="flex justify-between p-2 bg-gray-900/50 rounded">
                                <span className="text-gray-400">Número de Clubes:</span>
                                <span className="font-bold text-white">{selectedChampionship.clubs.length}</span>
                            </div>
                            <div className="flex justify-between p-2 bg-gray-900/50 rounded">
