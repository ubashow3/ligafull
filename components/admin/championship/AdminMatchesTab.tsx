...
import { Match, Club, League } from '../../../types';
import EditGameDetailsModal from '../../championship/EditGameDetailsModal';
...
const AdminMatchesTab: React.FC<AdminMatchesTabProps> = ({ matches, onSelectMatch, league, onUpdateMatch, clubs, onNavigateToCreateMatches }) => {
  ...
  const handleSaveChanges = (updatedMatch: Match) => {
  ...
  return (
    <div className="animate-fade-in">
      ...
      {matches.length === 0 && clubs.length >= 2 && (
        <div className="bg-gray-800 p-4 rounded-lg mb-6 text-center">
            ...
            <p className="text-gray-400 text-sm mb-4">Use o assistente para gerar o calendário de partidas para os {clubs.length} clubes inscritos.</p>
             <button
                onClick={onNavigateToCreateMatches}
      ...
      {matches.length === 0 ? (
        <div className="text-center py-10 text-gray-400">
            ...
            {clubs.length < 2 && " Adicione pelo menos 2 clubes para gerar os jogos."}
        </div>
      ) : (
            ...
            <div key={match.id} className="bg-gray-700/50 rounded-lg p-4">
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="text-xl sm:text-2xl font-bold text-white text-center">
                  {(match.status === 'finished' || match.status === 'in_progress') && match.homeScore != null && match.awayScore != null ? `${match.homeScore} x ${match.awayScore}` : 'x'}
                </div>
                <div className="flex flex-col items-center text-center w-2/5">
              ...
              <div className="text-center mt-4 flex justify-center items-center gap-2">
                ...
                <button 
                    ...
                    className="text-sm bg-gray-600 hover:bg-gray-500 text-blue-400 font-semibold py-1 px-3 rounded-full transition-colors"
                >
                    Detalhes
                </button>
              </div>
